...
type PurchaseOrderDetail = Omit<PurchaseOrderListItem, "frontImageUrl"> & {
  rawOcr: string;
  canMutate: boolean;
  images: {
    id: string;
...
type ListResponse = {
  documents?: PurchaseOrderListItem[];
  hasMore?: boolean;
...
function ownerLabel(item: Pick<PurchaseOrderListItem, "ownerUserId">, userId: string) {
  return item.ownerUserId === userId ? "自分" : "会社共有";
}

export function PurchaseOrderAlbum({ documentType, userId, initialOpenId }: PurchaseOrderAlbumProps) {
  ...
  const syncDraft = useCallback((doc: PurchaseOrderDetail) => {
